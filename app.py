...
def username():
	names = user_ids.keys()
	return render_template('../web_app/templates/username.html', names=names)

@app.route('/choose-hike', methods=['GET', 'POST'])
def enter_hike():
	hikes = list_hikes(sf_hikes)
	return render_template('../web_app/templates/choose-hike.html', hikes=hikes)
...
def enter_username():
	...
	try:
		...
		return render_template('../web_app/templates/error.html')	
	if username == '':
		return render_template('../web_app/templates/error.html')
	else:
		...
		return render_template('../web_app/templates/personal-recs.html', hike_data=hike_data)
...
def get_recommendations():
	...
	if hike == '':
		...
		return render_template('../web_app/templates/make-recommendations.html', your_hike=your_hike, hike_data=hike_data)
...
def get_popular():
    ...
    return render_template('../web_app/templates/most-reviewed.html', best_hikes=best_hikes)
...
def get_highest():
    ...
    for h_id in rec[0:10]:
        ...
        recs.append(hike)   
    best_hikes = top_five(recs)
    return render_template('../web_app/templates/highest-rated.html', best_hikes=best_hikes)
...
def get_power():
	...
	for h_id in power_ratings[0:10]:
		...
		recs.append(hike)
	best_hikes = top_five(recs)	
	return render_template('../web_app/templates/power-ratings.html', best_hikes=best_hikes)
